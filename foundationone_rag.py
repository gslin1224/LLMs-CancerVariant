...
total_epochs = 2
log_interval = 5  
models = ["qwen2.5:72b"]  # Using the original model list

# Directory for log files
log_dir = "/path/FoundationOneCivic_RAG"
...
for model in models:
        ...
        for epoch in range(epoch_start, epoch_end + 1):
            ...
            for batch_start in range(0, len(queries), batch_size):
                ...
                for i, query in enumerate(batch_queries):
                    question_number = batch_start + i + 1  # Get current query number
                    payload = {
                        "model": model,
                        "knowledgebaseId": 159,
                        "messages": [
    ...
    # Save results
    output_file_path = os.path.join(log_dir, f"foundation_LLM_onlyRAG_{model.replace(':', '_')}_epoch8~9.xlsx")
