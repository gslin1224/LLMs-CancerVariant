...
if 'Level' in data.columns:
    ...
    original_levels = ["N/A"] * len(data)  # Fill with N/A if the original file has no Level column
...
for model in models:
            ...
            for i, query in enumerate(batch_queries):
                ...
                if response.status_code == 200:
                    try:
                        ...
                        batch_responses.append("Response decode error")
                        print(f"Model {model} - Epoch {epoch} - Response decode error for query {batch_start + i + 1}")
                else:
    ...
    output_file_path = f'/path/LLM_{model.replace(":", "_")}_all_epochs_response.xlsx'
