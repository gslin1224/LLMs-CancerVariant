...
# Load the CIViC dataset from an Excel file
file_path = '/path/CIViC_01-Nov-2024-ClinicalEvidenceSummaries_SelectedForLLMExplanation.xlsx'
data = pd.read_excel(file_path)

# Select relevant columns for building queries
selected_columns = data[['molecular_profile', 'disease']]
if 'evidence_level' in data.columns:
    original_levels = data['evidence_level'].tolist()
else:
    original_levels = ["N/A"] * len(data)  # If 'evidence_level' column is missing, fill with "N/A"

# Build query list based on the selected columns
queries = []
...
# List of models to use for the queries
models = ["gpt-4", "qwen2.5:72b", "llama3.1:70b"]
log_dir = "/path/gene_explain/civic"
...
# Batch size for sending requests
batch_size = 5
...
for model in models:
    ...
    log_file_path = os.path.join(log_dir, f"civic_LLM_log_{model}.log")
...
output_file_path = os.path.join(log_dir, "civic_LLM_results_combined.xlsx")
results_df.to_excel(output_file_path, index=False)
print(f"All model results have been saved to: {output_file_path}")
